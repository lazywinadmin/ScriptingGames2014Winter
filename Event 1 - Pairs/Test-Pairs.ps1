...
#>
    ...
    PROCESS {
        TRY{
			...
			Send-MailMessage -From $EmailFrom -To $EmailTo -Cc $EmailCc -Subject $EmailSubject -Body "Hello dear fellow scripter ! Find here the fantastic list of pairs that has been generated by the PoshMonks. $($pairs)" -BodyAsHtml -SmtpServer $EmailServer
		}#TRY
		CATCH {
			Write-Warning -Message "[PROCESS] Something wrong happened !"
			Write-Warning -Message $Error[0].Exception
